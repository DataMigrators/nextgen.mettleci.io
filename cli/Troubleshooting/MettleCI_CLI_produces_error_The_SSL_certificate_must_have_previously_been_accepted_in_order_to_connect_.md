...
…works well until it gets to the compilation step at which point each
job produces this error:
...
`The SSL certificate must have previously been accepted in order to connect`
is to <a
href="https://www.ibm.com/support/pages/certificate-warning-when-launching-datastage-client-tools-right-after-information-server-client-installation"
rel="nofollow">ensure you have accepted your SSL certificate</a>.

# In summary

Ensure your DataStage environment is properly installed, configured, and
functional before using the MettleCI CLI. In particular ensure your
`istool` and `dscc` commands behave correctly.
